...
class SavedRepository {
  ...
  set resonators(List<JsonType> jsonResonator) => _savedData["resonators"] = jsonResonator;
  set weapons(List<JsonType> jsonWeapons) => _savedData["weapons"] = jsonWeapons;
  ...
  void loadData([String? path]) {
    ...
    if (_savedFile.readAsStringSync().isEmpty) {
      _savedFile.writeAsStringSync(jsonEncode({"resonators": [], "echoes": [], "weapons": []}));
    }
  ...
  void saveData() => _savedFile.writeAsString(jsonEncode(_savedData));

  void _writeDefaultValues() {
    try {
      // Resonator first
      Directory(assetDir.join("resonators")).listSync().whereType<File>().forEach((f) {
        _log.debug("Reading ${f.path}");
        final _json = jsonDecode(f.readAsStringSync());
        Resonator newResonator = Resonator(
          name: _json['name'],
          elementType: ElementType.values.firstWhere((e) => e.name == _json["element"]),
          weaponType: WeaponType.values.firstWhere((w) => w.name == _json["weaponType"]),
        );
        (_savedData["resonators"] as List).add(newResonator.toJson());
      });

      // Then Weapon
      Directory(assetDir.join("weapons")).listSync().whereType<File>().forEach((f) {
        _log.debug("Reading ${f.path}");
        final _json = jsonDecode(f.readAsStringSync());
